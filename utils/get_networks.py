...
def get_network(args):
    ...
    if dataset in {'cifar10', 'cifar100'}:
        ...
        if args.network.lower() == 'preresnet':
            ...
            net = get_network_cifar(args.network, depth=args.depth, alpha=48,
                                    input_shape=(1, args.in_channels, args.input_dim, args.input_dim),
                                    num_classes=args.outputs_dim, base_channels=16, block_type='bottleneck')
        elif args.network.lower() in ['vit_small', 'vit_base', 'vit_large', 'vit_huge']:
            net = get_network_cifar(args.network, mlp_ratio=args.mlp_ratio, input_size=args.input_dim,
                                    patch_size=args.patch_size, in_channels=args.in_channels,
                                    num_classes=args.outputs_dim)
        else:
            net = get_network_cifar(args.network, depth=args.depth, num_classes=args.outputs_dim,
                                    growthRate=args.growthRate, compressionRate=args.compressionRate,
                                    widen_factor=args.widen_factor, dropRate=args.dropRate)
    if dataset == 'svhn':
        ...
        if args.network.lower() == 'preresnet':
            ...
            net = get_network_svhn(args.network, depth=args.depth, alpha=48,
                                   input_shape=(1, args.in_channels, args.input_dim, args.input_dim),
                                   num_classes=args.outputs_dim, base_channels=16, block_type='bottleneck')
        elif args.network.lower() in ['vit_small', 'vit_base', 'vit_large', 'vit_huge']:
            net = get_network_svhn(args.network, mlp_ratio=args.mlp_ratio, input_size=args.input_dim,
                                   patch_size=args.patch_size, in_channels=args.in_channels,
                                   num_classes=args.outputs_dim)
        else:
            net = get_network_svhn(args.network, depth=args.depth, num_classes=args.outputs_dim,
    ...
    if dataset == 'imagenet':
        ...
        if args.network.lower() == 'pyramidnet':
            net = get_network_imagenet(args.network, depth=args.depth, alpha=48,
                                       input_shape=(1, args.in_channels, args.input_dim, args.input_dim),
                                       num_classes=args.outputs_dim, base_channels=16, block_type='bottleneck')
        elif args.network.lower() in ['vit_small', 'vit_base', 'vit_large', 'vit_huge']:
            net = get_network_imagenet(args.network, mlp_ratio=args.mlp_ratio, input_size=args.input_dim,
                                       patch_size=args.patch_size, in_channels=args.in_channels,
                                       num_classes=args.outputs_dim)
            ...
            net = get_network_imagenet(args.network, depth=args.depth, num_classes=args.outputs_dim,
                                       growthRate=args.growthRate, compressionRate=args.compressionRate,
                                       widen_factor=args.widen_factor, dropRate=args.dropRate)
    if dataset == 'miniimagenet':
        from utils.network_utils_miniimagenet import get_network_miniimagenet
        if args.network.lower() == 'pyramidnet':
            net = get_network_miniimagenet(args.network, depth=args.depth, alpha=48,
                                           input_shape=(1, args.in_channels, args.input_dim, args.input_dim),
                                           num_classes=args.outputs_dim, base_channels=16, block_type='bottleneck')
        elif args.network.lower() in ['vit_small', 'vit_base', 'vit_large', 'vit_huge']:
            net = get_network_miniimagenet(args.network, mlp_ratio=args.mlp_ratio, input_size=args.input_dim,
                                           patch_size=args.patch_size, in_channels=args.in_channels,
                                           num_classes=args.outputs_dim)
        elif args.network.lower() in ['pretrain_vit', 'pretrain_swin', 'pretrain_deit', 'pretrain_beit', 'pretrain_pvt']:
            net = get_network_miniimagenet(args.network, num_classes=args.outputs_dim)
        else:
            net = get_network_miniimagenet(args.network, depth=args.depth, num_classes=args.outputs_dim,
                                           growthRate=args.growthRate, compressionRate=args.compressionRate,
                                           widen_factor=args.widen_factor, dropRate=args.dropRate)
    if dataset == 'tinyimagenet':
        from utils.network_utils_tinyimagenet import get_network_tinyimagenet
        if args.network.lower() == 'pyramidnet':
            net = get_network_tinyimagenet(args.network, depth=args.depth, alpha=48,
                                           input_shape=(1, args.in_channels, args.input_dim, args.input_dim),
                                           num_classes=args.outputs_dim, base_channels=16, block_type='bottleneck')
        elif args.network.lower() in ['vit_small', 'vit_base', 'vit_large', 'vit_huge']:
            net = get_network_tinyimagenet(args.network, mlp_ratio=args.mlp_ratio, input_size=args.input_dim,
                                           patch_size=args.patch_size, in_channels=args.in_channels,
                                           num_classes=args.outputs_dim)
        elif args.network.lower() in ['pretrain_vit', 'pretrain_swin', 'pretrain_deit', 'pretrain_beit', 'pretrain_pvt']:
            net = get_network_tinyimagenet(args.network, num_classes=args.outputs_dim)
        else:
            net = get_network_tinyimagenet(args.network, depth=args.depth, num_classes=args.outputs_dim,
                                           growthRate=args.growthRate, compressionRate=args.compressionRate,
                                           widen_factor=args.widen_factor, dropRate=args.dropRate)
    if dataset in TEXT_DATASETS:
        from utils.network_utils_text import get_network_text
        net = get_network_text(args.network, model_name=args.text_model_name, num_classes=args.outputs_dim,
